...
class CronJobController extends Controller
{
    ...
    {
        $rank = Ranking::where('status', true)->get(['id', 'bonus', 'minimum_earnings', 'min_invest', 'team_min_invest', 'min_referral']);

        User::chunk(200, function ($users) use ($rank) {
            foreach ($users as $user) {
                $totalEarning = $user->totalProfit();
                $totalInvestment = $user->totalInvestment();
                $data = $user->getReferrals()->first();
                $referral = $data->relationships()->count();
                $teamReferral = $user->referrals();

                $ranks = $rank->whereNotIn('id', json_decode($user->rankings))->where('minimum_earnings', '<=', $totalEarning)->where('min_invest', '<=', $totalInvestment)->where('min_referral', '<=', $referral)->where('team_min_invest', '<=', $teamReferral);

                $maxRank = $ranks->max('minimum_earnings');
                $maxInvest = $ranks->max('min_invest');
                $maxReffer = $ranks->max('min_referral');
                $maxTeamReffer = $ranks->max('team_min_invest');
                ...
                foreach ($ranks as $rank) {
                    ...
                    if ($rank->minimum_earnings == $maxRank && $rank->min_invest == $maxInvest && $rank->min_referral == $maxReffer && $rank->team_min_invest == $maxTeamReffer) {
